...
describe(@"postDictionaryForText", ^{
    ...
    describe(@"more than 2 markdown URLs", ^{
        it(@"should return the correct text for 3 urls", ^{
            NSString *post = @"Foo [URL](google.com) bar [Github](github.com) baz [Gmail](gmail.com) qux";
        ...
        it(@"should return the correct text for 4 urls", ^{
            NSString *post = @"Foo [URL](google.com) bar [Github](github.com) baz [Gmail](gmail.com) qux [ADN](app.net) quux";
    ...
    it(@"should have a correct error", ^{
        NSString *post = @"A string with a [@username](url)";
        NSError *error = nil;
        NSDictionary *dictionary = [[KSADNPostParser shared] postDictionaryForText:post error:&error];
        expect(dictionary).to.equal(nil);
        expect(error).notTo.equal(nil);
